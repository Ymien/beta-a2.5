      ...
      <div className="border-b border-black/10">
        <div className="mx-auto flex h-8 max-w-6xl items-center justify-between px-4 text-[11px] md:px-6">
          <div className="flex items-center gap-2 text-black/45">
            <span className="font-medium tracking-[0.22em] uppercase">Blank Space</span>
            <span className="text-black/25">—</span>
            <span>{lang === "zh" ? "轻写 · 轻做" : "Write lightly · Make lightly"}</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={toggleLang}
              className="rounded-full border border-black/10 bg-white/70 px-3 py-1 font-medium text-black/55 transition hover:bg-white"
            >
              {lang === "zh" ? "EN" : "中文"}
            </button>
          </div>
